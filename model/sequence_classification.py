...
from transformers import MT5EncoderModel, MT5Config, T5PreTrainedModel
...
class RobertaPrefixForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        #print(input_ids[0], outputs[0].size(), outputs[1].size())
        pooled_output = outputs[1]
    ...
    def hiddens(
        ...
        past_key_values = self.get_prompt(batch_size=batch_size)
        prefix_attention_mask = torch.ones(batch_size, self.pre_seq_len).to(self.roberta.device)
        #print(prefix_attention_mask, attention_mask.to(self.roberta.device))
        attention_mask = torch.cat((prefix_attention_mask, attention_mask), dim=1)
