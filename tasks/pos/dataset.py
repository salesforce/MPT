...
class POSDataset():
    def __init__(self, tokenizer: AutoTokenizer, data_args, training_args) -> None:
        ...
        raw_datasets = load_dataset(f'tasks/pos/datasets/{data_args.dataset_name}.py', name=data_args.lang)

        self.tokenizer = tokenizer

        #if training_args.do_train:
            # Lifu: filter too long sequence
            #raw_datasets["train"] = raw_datasets["train"].filter(lambda x: len(x["tokens"]) < 128 )
        ...
        if training_args.do_eval:
            ...
            if training_args.do_train:
                ...
                eval_dataset = eval_dataset.filter(lambda x: len(x["tokens"]) < 128 )
        ...
        if training_args.do_predict:
            ##Lifu
            raw_datasets["test"] = raw_datasets["test"].filter(lambda x: len(x["tokens"]) < 128 )
