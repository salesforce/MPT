    ...
    def __init__(self, tokenizer: AutoTokenizer, data_args, training_args) -> None:
        ...
        if training_args.do_eval:
            if training_args.do_train:
                ...
                self.eval_dataset = raw_datasets["test_matched" if data_args.dataset_name == "mnli" else "test"]
            if data_args.max_eval_samples is not None:
                self.eval_dataset = self.eval_dataset.select(range(data_args.max_eval_samples))
            ### using training set for analysis
            if data_args.do_analysis:
                self.eval_dataset = raw_datasets['train'].select(range(2000))
        ...
        #print(self.eval_dataset[:5])
        if data_args.dataset_name=="xnli":
            self.metric = load_metric("glue", 'mnli')
        elif data_args.dataset_name=="paws-x":
            ## did not set metric here. Use accuracy in function "compute_metrics" 
            self.metric = None
